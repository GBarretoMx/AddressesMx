...
class AddressesByState implements ResolverInterface
{
    ...
    )
    {
        if (empty($args['estado'])) {
            throw new GraphQlInputException(__('Estado must be specified'));
        }
        $addressesCollection = $this->addressesCollectionFactory->create()
            ->addFieldToSelect('*')
            ->addFieldToFilter('estado', ['eq' => $args['estado']])
            ->getData();

        if (empty($addressesCollection)) {
            throw new GraphQlInputException(__('No match was found with Estado'));
        }
